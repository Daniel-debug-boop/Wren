...
# ---------------------------------------------------------------------------
# The ``wren-sdk`` package is the **single source of truth** for which
...
def is_wren_model(model: str | None) -> bool:
    """Return True when the model uses the public OpenHands provider prefix."""
    return bool(model and model.startswith('wren/'))
...
def resolve_llm_base_url(
    ...
    * ``wren/app_server/settings/settings_router._post_merge_llm_fixups``
...
def _derive_verified_models(wren_models: list[str]) -> list[str]:
    """Extract the bare model names from the ``wren/…`` model list."""
    return [
        m.removeprefix('wren/')
        for m in wren_models
        if m.startswith('wren/')
    ]
...
def get_supported_llm_models(
    ...
    * the recommended default model.
    ...
    wren_models = get_wren_models(verified_models)

    # Assign canonical provider prefixes to bare LiteLLM names, then dedupe.
    all_models = (
        wren_models + CLARIFAI_MODELS + [_assign_provider(m) for m in model_list]
    )
