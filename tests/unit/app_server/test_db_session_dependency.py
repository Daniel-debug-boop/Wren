"""Regression tests for ``wren.app_server.services.db_session``.
...
from wren.app_server.services.db_session import (
    _yield_db_session,
    depends_db_session,
)
...
def test_depends_db_session_does_not_resolve_config_at_construction():
    ...
    We install a sentinel ``wren.app_server.config`` module whose
    ...
    """
    original = sys.modules.get('wren.app_server.config')
    sentinel = types.ModuleType('wren.app_server.config')
    ...
    sys.modules['wren.app_server.config'] = sentinel
    try:
        ...
        if original is None:
            sys.modules.pop('wren.app_server.config', None)
        else:
            sys.modules['wren.app_server.config'] = original
