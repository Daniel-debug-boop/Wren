...
def test_localhost_cors_middleware_init_with_config():
    ...
    with patch(
        'wren.app_server.middleware.get_global_config', return_value=mock_config
    ):
        app = FastAPI()
...
def test_in_memory_rate_limiter_init():
    ...
    rate_limiter = InMemoryRateLimiter(requests=5, seconds=10)
...
def test_rate_limit_middleware_exempts_sandbox_resume():
    ...
    app.add_middleware(
        RateLimitMiddleware,
        rate_limiter=InMemoryRateLimiter(requests=0, seconds=60),
    )
    client = TestClient(app)
