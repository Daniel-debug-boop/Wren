"""Tests for resolve_provider_llm_base_url in wren.app_server.config."""
...
class TestOpenHandsPrefixWithProviderUrl:
    """wren/ prefix + SDK default URL + provider URL set → returns provider URL."""

    def test_wren_prefix_replaces_sdk_default(self):
    ...
    def test_wren_prefix_sdk_default_no_trailing_slash(self):
        result = resolve_provider_llm_base_url(
            model='wren/gpt-4',
...
class TestOpenHandsPrefixWithCustomUrl:
    """wren/ prefix + custom URL (not SDK default) → returns custom URL."""
...
class TestOpenHandsPrefixNoProviderUrl:
    """wren/ prefix + SDK default URL + no provider URL → returns SDK default."""
...
class TestNonMatchingModelPrefix:
    """Model without wren/ prefix → returns base_url unchanged."""
...
class TestEdgeCases:
    ...
    def test_none_base_url_with_wren_model_and_provider(self):
        result = resolve_provider_llm_base_url(
            model='wren/gpt-4',
            base_url=None,
            provider_base_url=STAGING_URL,
