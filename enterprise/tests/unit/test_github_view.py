...
class TestGithubLabels(TestCase):
    def test_labels_with_staging(self):
        ...
        self.assertEqual(oh_label, 'wren-exp')
        self.assertEqual(inline_oh_label, '@wren-exp')
    ...
    def test_labels_with_prod(self):
        ...
        self.assertEqual(oh_label, 'wren')
        self.assertEqual(inline_oh_label, '@wren')
    ...
    @mock.patch.dict('os.environ', {'OH_RESOLVER_LABEL': 'wren-dev'})
    def test_labels_with_override(self):
        """An explicit OH_RESOLVER_LABEL overrides host inference."""
        oh_label, inline_oh_label = get_oh_labels('app.all-hands.dev')
        self.assertEqual(oh_label, 'wren-dev')
        self.assertEqual(inline_oh_label, '@wren-dev')
...
class TestGithubCommentCaseInsensitivity(TestCase):
    @mock.patch('integrations.github.github_view.INLINE_OH_LABEL', '@wren')
    def test_issue_comment_case_insensitivity(self):
        ...
        message_lower = Message(
            ...
            message={
                'payload': {
                    'action': 'created',
                    'comment': {'body': 'hello @wren please help'},
                    'issue': {'number': 1},
                }
            },
        )
