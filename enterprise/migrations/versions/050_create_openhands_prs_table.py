...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('wren_helped_author', sa.Boolean(), nullable=True),
        sa.Column('num_wren_commits', sa.Integer(), nullable=True),
        sa.Column('num_wren_review_comments', sa.Integer(), nullable=True),
        sa.Column('num_wren_general_comments', sa.Integer(), nullable=True),
    )

    # Create indexes for efficient querying
    op.create_index(
        'ix_wren_prs_repo_id', 'wren_prs', ['repo_id'], unique=False
    )
    op.create_index(
        'ix_wren_prs_pr_number', 'wren_prs', ['pr_number'], unique=False
    )
    op.create_index(
        'ix_wren_prs_status', 'wren_prs', ['status'], unique=False
    )

    # Create unique constraint on repo_id + pr_number combination
    op.create_index(
        'ix_wren_prs_repo_pr_unique',
        'wren_prs',
        ['repo_id', 'pr_number'],
        unique=True,
    )
...
def downgrade() -> None:
    op.drop_index('ix_wren_prs_repo_id', table_name='wren_prs')
    op.drop_index('ix_wren_prs_pr_number', table_name='wren_prs')
    op.drop_index('ix_wren_prs_status', table_name='wren_prs')
    op.drop_index('ix_wren_prs_repo_pr_unique', table_name='wren_prs')
    op.drop_table('wren_prs')
