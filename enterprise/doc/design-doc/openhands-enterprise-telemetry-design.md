# Wren Enterprise Usage Telemetry Service
...
5. [Implementation Plan](#5-implementation-plan)
   - 5.1 [Database Schema and Models (M1)](#51-database-schema-and-models-m1)
     - 5.1.1 [Wren - Database Migration](#511-wren---database-migration)
     - 5.1.2 [Wren - Model Tests](#512-wren---model-tests)
   - 5.2 [Metrics Collection Framework (M2)](#52-metrics-collection-framework-m2)
     - 5.2.1 [Wren - Core Collection Framework](#521-wren---core-collection-framework)
     - 5.2.2 [Wren - Example Collectors](#522-wren---example-collectors)
     - 5.2.3 [Wren - Framework Tests](#523-wren---framework-tests)
   - 5.3 [Collection and Upload Processors (M3)](#53-collection-and-upload-processors-m3)
     - 5.3.1 [Wren - Collection Processor](#531-wren---collection-processor)
     - 5.3.2 [Wren - Upload Processor](#532-wren---upload-processor)
     - 5.3.3 [Wren - Integration Tests](#533-wren---integration-tests)
   - 5.4 [License Warning API (M4)](#54-license-warning-api-m4)
     - 5.4.1 [Wren - License Status API](#541-wren---license-status-api)
     - 5.4.2 [Wren - API Integration](#542-wren---api-integration)
   - 5.5 [UI Warning Banner (M5)](#55-ui-warning-banner-m5)
     - 5.5.1 [Wren - UI Warning Banner](#551-wren---ui-warning-banner)
     - 5.5.2 [Wren - UI Integration](#552-wren---ui-integration)
   - 5.6 [Helm Chart Deployment Configuration (M6)](#56-helm-chart-deployment-configuration-m6)
     - 5.6.1 [Wren-Cloud - Cronjob Manifests](#561-wren-cloud---cronjob-manifests)
     - 5.6.2 [Wren-Cloud - Configuration Management](#562-wren-cloud---configuration-management)
   - 5.7 [Documentation and Enhanced Collectors (M7)](#57-documentation-and-enhanced-collectors-m7)
     - 5.7.1 [Wren - Advanced Collectors](#571-wren---advanced-collectors)
     - 5.7.2 [Wren - Monitoring and Testing](#572-wren---monitoring-and-testing)
     - 5.7.3 [Wren - Technical Documentation](#573-wren---technical-documentation)

## 1. Introduction

### 1.1 Problem Statement

Wren Enterprise (OHE) helm charts are publicly available but not open source, creating a visibility gap for the sales team. Unknown users can install and use OHE without the vendor's knowledge, preventing proper customer engagement and sales pipeline management. Without usage telemetry, the vendor cannot identify potential customers, track installation health, or proactively support users who may need assistance.
...
When telemetry uploads fail for more than 4 days, users will see a prominent warning banner in the Wren Enterprise UI:

```
⚠️ Your Wren Enterprise license will expire in 30 days. Please contact support if this issue persists.
```
...
async def get_license_status():
    ...
    async with a_session_maker() as session:
        ...
        if not last_upload:
            ...
            return {
                "warn": True,
                "message": "Wren Enterprise license verification pending. Please ensure network connectivity."
            }
        ...
        if days_since_upload > 4:
            ...
            if oldest_unsent:
                ...
                if days_until_expiration <= 0:
                    message = "Your Wren Enterprise license has expired. Please contact support immediately."
                else:
                    message = f"Your Wren Enterprise license will expire in {days_until_expiration} days. Please contact support if this issue persists."

                return {"warn": True, "message": message}

        return {"warn": False, "message": ""}

def _is_wren_enterprise() -> bool:
...
The cronjob configurations will be deployed via the Wren-Cloud helm charts.
...
```yaml
# charts/wren/templates/telemetry-collection-cronjob.yaml
apiVersion: batch/v1
kind: CronJob
metadata:
  name: {{ include "wren.fullname" . }}-telemetry-collection
  labels:
    {{- include "wren.labels" . | nindent 4 }}
spec:
...
```yaml
# charts/wren/templates/telemetry-upload-cronjob.yaml
apiVersion: batch/v1
kind: CronJob
metadata:
  name: {{ include "wren.fullname" . }}-telemetry-upload
  labels:
    {{- include "wren.labels" . | nindent 4 }}
spec:
        ...
        spec:
          ...
          - name: telemetry-uploader
            ...
            - name: REPLICATED_PUBLISHABLE_KEY
              valueFrom:
                secretKeyRef:
                  name: {{ include "wren.fullname" . }}-replicated-config
                  key: publishable-key
            - name: REPLICATED_APP_SLUG
              value: {{ .Values.telemetry.replicatedAppSlug | default "wren-enterprise" | quote }}
            command:
            - python
            - -c
            - |
              ...
              async def main():
                  ...
                  processor = TelemetryUploadProcessor(
                      ...
                      replicated_app_slug=os.getenv('REPLICATED_APP_SLUG', 'wren-enterprise')
                  )
                  task = MaintenanceTask()
                  task.set_processor(processor)
                  task.status = MaintenanceTaskStatus.PENDING
...
#### 5.2.1 Wren - Core Collection Framework
...
**Repository**: Wren-Cloud
Create Kubernetes cronjob configurations and deployment scripts.

#### 5.6.1 Wren-Cloud - Cronjob Manifests

- [ ] `charts/wren/templates/telemetry-collection-cronjob.yaml`
- [ ] `charts/wren/templates/telemetry-upload-cronjob.yaml`

#### 5.6.2 Wren-Cloud - Configuration Management

- [ ] `charts/wren/templates/replicated-secret.yaml`
- [ ] Update `charts/wren/values.yaml` with telemetry configuration options:
  ```yaml
  # Add to values.yaml
  telemetry:
    enabled: true
    replicatedAppSlug: "wren-enterprise"
...
#### 5.7.1 Wren - Advanced Collectors
...
#### 5.7.2 Wren - Monitoring and Testing
...
#### 5.7.3 Wren - Technical Documentation
