...
from wren.app_server.utils.llm import ModelsResponse, get_supported_llm_models
...
# ``wren/`` -> ``litellm_proxy/`` at the transport boundary.
_OPENHANDS_PREFIX = 'wren/'
...
def _derive_default_model() -> str:
    ...
    the app stores and displays models as ``wren/<name>``. An already-
    ``wren/``-prefixed default is kept as-is; a bare name is prefixed.
    """
...
class LiteLLMProxyModelService(DefaultLLMModelService):
    ...
    async def _fetch_proxy_model_names(
        ...
        Entries flagged ``model_info.wren_hidden`` (legacy alias routes
        ...
        ``model_info.wren_canonical`` naming the visible route it
        ...
        for entry in payload.get('data') or []:
            ...
            hidden = isinstance(model_info, dict) and bool(
                model_info.get('wren_hidden')
            )
            if hidden and name not in canonical_by_name:
                canonical = model_info.get('wren_canonical')
    ...
    def _is_model_verified(
        ...
        # is not. Require the wren/ prefix so a catalogue model whose bare
