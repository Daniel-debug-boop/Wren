...
* ``wren_user_id`` — set once ``UserStore.create_user`` returns,
...
async def provision_user(
    ...
    try:
        ...
        if target_org_id != wren_user_id:
            ...
            # Track separately from ``wren_user_id`` so the unwind
...
async def _rollback_partial_provision(
    *,
    ...
    if wren_user_id is not None and target_membership_added:
        try:
            await OrgMemberStore.remove_user_from_org(target_org_id, wren_user_id)
        except Exception:
            logger.exception(
    ...
    if wren_user_id is not None:
        try:
            await OrgStore.delete_org_cascade(
                wren_user_id, requester_user_id=kc_user_id
            )
        except Exception:
            logger.exception(
