...
from storage.lite_llm_manager import LiteLlmManager, get_wren_cloud_key_alias
...
from wren.app_server.utils.jsonpatch_compat import (
    WHOLESALE_REPLACEMENT_KEYS,
    deep_merge,
    deep_merge_with_wholesale_keys,
)
from wren.app_server.utils.llm import is_wren_model
from wren.sdk.llm.utils.wren_provider import (
    canonicalize_wren_llm_payload,
)
...
class SaasSettingsStore(SettingsStore):
    ...
    async def load(self) -> Settings | None:
        ...
        if isinstance(llm_dict, dict):
            merged_agent_settings['llm'] = canonicalize_wren_llm_payload(llm_dict)
        ...
        if org.llm_profiles:
            ...
            if isinstance(raw_profiles, dict):
                profiles_data['profiles'] = {
                    name: canonicalize_wren_llm_payload(prof)
    ...
    async def store(self, item: Settings):
        async with a_session_maker() as session:
            ...
            uses_managed_llm_key = (
                ...
                or (normalized_llm_base_url is None and is_wren_model(llm_model))
            )

            if uses_managed_llm_key:
                await self._ensure_api_key(
                    item, str(org_id), wren_type=is_wren_model(llm_model)
                )
    ...
    async def _ensure_api_key(
        self, item: Settings, org_id: str, wren_type: bool = False
    ) -> None:
        ...
        if llm_api_key and not await LiteLlmManager.verify_existing_key(
            ...
            # the default to/from an wren/* model never orphans a key.
            key_alias = get_wren_cloud_key_alias(self.user_id, org_id)
            await LiteLlmManager.delete_key_by_alias(key_alias=key_alias)
            generated_key = await LiteLlmManager.generate_key(
                self.user_id,
                org_id,
                key_alias,
                {'type': 'wren'} if wren_type else None,
            )

            item.agent_settings.llm.api_key = SecretStr(generated_key)
            logger.info(
                'saas_settings_store:store:generated_wren_key',
