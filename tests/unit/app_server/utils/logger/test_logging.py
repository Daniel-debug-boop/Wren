...
from wren.app_server.utils.logger import (
    LOG_JSON_LEVEL_KEY,
    OpenHandsLoggerAdapter,
    json_log_handler,
)
from wren.app_server.utils.logger import wren_logger as wren_logger
...
def test_handler():
    ...
    wren_logger.addHandler(handler)
    yield wren_logger, stream
    wren_logger.removeHandler(handler)
...
def json_handler():
    ...
    wren_logger.addHandler(json_handler)
    yield wren_logger, stream
    wren_logger.removeHandler(json_handler)
...
def test_sensitive_env_vars_masking(test_handler):
    ...
    with patch.dict(
        'wren.app_server.utils.logger.os.environ', environ, clear=True
    ):
