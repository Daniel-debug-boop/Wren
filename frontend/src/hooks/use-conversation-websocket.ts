    ...
    (data: WebSocketEvent) => {
      ...
      switch (data.type) {
        ...
        case "action": {
          ...
          if (
            currentMode === "plan" &&
            data.action?.thought?.toLowerCase().includes("plan:")
          ) {
            const { thought } = data.action;
        ...
        case "observation": {
          ...
          if (
            currentMode === "debug" &&
            (obsContent.includes("Error:") || obsContent.includes("Traceback"))
          ) {
            const errMsg =
              obsContent.match(/Error:\s*(.+?)(?:\n|$)/)?.[1] ||
              obsContent.slice(0, 200);
            h.onDebugError({
              message: errMsg,
              type: obsContent.includes("Traceback")
                ? "Traceback"
                : "RuntimeError",
              stack: obsContent,
            });
            const lines = obsContent
              .split("\n")
              .filter(
                (l) =>
        ...
        case "status":
          if (data.status?.execution_status === "running") {
            ...
            ["awaiting_user_input", "paused", "stopped"].includes(
              data.status?.execution_status ?? "",
            )
          ) {
          ...
          addTimelineEvent({
            ...
            status:
              data.status?.execution_status === "running" ? "running" : "done",
          });
          break;
