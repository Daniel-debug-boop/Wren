...
from wren.app_server.user_auth import get_user_auth, get_user_id
...
async def get_admin_user_id(
    ...
    """
    Dependency that validates user has @wren.dev email domain.
    ...
    by checking for @wren.dev email domain.
    ...
    Raises:
        HTTPException: 403 if email domain is not @wren.dev
    ...
    if not user_email.endswith('@wren.dev'):
        logger.warning(
        ...
        raise HTTPException(
            ...
            detail='Access restricted to @wren.dev users',
        )
...
async def get_org_creator_user_id(
    ...
    - When **disabled** (the default), this falls back to ``get_admin_user_id``
      so org creation remains restricted to ``@wren.dev`` admin users.
