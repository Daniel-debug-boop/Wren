...
class TestGetVersionFromPyproject:
    ...
    def test_reads_first_candidate_path(self, tmp_path):
        ...
        fake_file = tmp_path / 'wren' / 'app_server' / 'version.py'
        fake_file.parent.mkdir(parents=True)
        fake_file.touch()

        with patch(
            'wren.app_server.version.os.path.abspath', return_value=str(fake_file)
        ):
            assert get_version() == '1.2.3'

    def test_reads_second_candidate_path(self, tmp_path):
        # Only place pyproject.toml under the wren/ subdirectory
        wren_dir = tmp_path / 'wren'
        wren_dir.mkdir()
        _write_pyproject(wren_dir, '4.5.6')
...
class TestGetVersionFallbacks:
    ...
    def _patch_no_pyproject(self):
        ...
        return patch(
            'wren.app_server.version.os.path.abspath',
            return_value='/nonexistent/wren/app_server/version.py',
        )
...
class TestModuleLevelVersion:
    ...
    def test_module_version_matches_pattern(self):
        from wren.app_server.version import __version__
