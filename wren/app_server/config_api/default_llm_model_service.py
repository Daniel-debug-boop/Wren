...
def _to_providers(models_response: ModelsResponse) -> list[Provider]:
    """Extract unique providers, sorted with ``wren`` first, then other
    ...
    # ``wren`` is the managed provider and should always appear first,
    # followed by other verified providers (alphabetical), then unverified
    # providers (alphabetical).
    providers.sort(key=lambda p: (not p.verified, p.name != 'wren', p.name))
    return providers
