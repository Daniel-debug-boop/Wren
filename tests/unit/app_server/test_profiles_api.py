...
from wren.app_server.file_store import get_file_store
...
from wren.app_server.settings.llm_profiles import MAX_PROFILES_PER_USER
from wren.app_server.settings.settings_models import Settings
from wren.app_server.settings.settings_router import _user_profile_locks
...
async def test_activate_profile_applies_base_url_fixup(test_client, settings_store):
    ...
    settings.llm_profiles.save(
        'oh-profile',
        LLM(model='wren/claude-sonnet-4-20250514'),
    )
    await _seed(settings_store, settings)
...
async def test_activate_does_not_mutate_saved_profile_base_url(
    ...
    Use a custom base_url (not an wren/* model) so the LLM SDK does
...
async def test_concurrent_writes_all_persist(tmp_path: Path):
    ...
    from wren.app_server.settings.settings_router import (
        SaveProfileRequest,
        save_profile,
    )
