...
from wren.app_server.utils.auth import looks_like_jwt
...
def get_project_dir(
    ...
    `.wren/` configuration (setup.sh, pre-commit.sh, skills/, etc.).
    ...
    - .wren/setup.sh execution
    - .wren/pre-commit.sh (git hooks setup)
    - .wren/skills/ (project skills)
    - PLAN.md path
...
class AppConversationServiceBase(AppConversationService, ABC):
    ...
    async def load_and_merge_all_skills(
        ...
        - User skills (from ~/.wren/skills/)
        - Organization skills (from {org}/.wren repo)
        - Project/repo skills (from repo .agents/skills/, .wren/microagents/, and legacy .wren/skills/)
        ...
        async def _background_tool_check():
            try:
                ...
                if report.get('missing'):
                    _logger.info(
                        'ToolRegistry: %d missing capabilities detected. '
                        'Call POST /api/v1/tool-registry/analyze with '
                        'auto_install=true to install them.',
                        len(report['missing']),
                    )
            except Exception:
                _logger.debug('ToolRegistry background check skipped', exc_info=True)

        asyncio.ensure_future(_background_tool_check())
    ...
    async def run_setup_scripts(
        ...
        # for all .wren/ features (setup.sh, pre-commit.sh, skills).
    ...
    async def clone_or_init_git_repo(
        ...
        if request.selected_branch:
            ...
            wren_workspace_branch = f'wren-workspace-{random_str}'
            checkout_command = (
                f'git checkout -b {shlex.quote(wren_workspace_branch)}'
            )
    ...
    async def _configure_azure_devops_git_credential_helper(
        ...
        helper_path = git_dir / '.git' / 'wren-azure-devops-credential-helper'
        secret_path = (
...
"""
    ...
    async def maybe_setup_git_hooks(
        ...
        """Set up git hooks if .wren/pre-commit.sh exists in the project root.

        Args:
            workspace: Remote workspace for command execution.
            project_dir: Project root directory (repo root when a repo is selected).
        """
        command = 'mkdir -p .git/hooks && chmod +x .wren/pre-commit.sh'
