...
Common utility functions and helpers for Wren app server.

## Overview

This module provides utility functions that are used across Wren for common operations like date handling, SQL operations, dynamic imports, async utilities, LLM integration, and more.
...
Wren provides an extensibility mechanism through the `get_impl` and `import_from` functions in `import_utils.py`. This mechanism allows applications built on Wren to customize behavior by providing their own implementations of Wren base classes.

### How It Works

1. Base classes define interfaces through abstract methods and properties
2. Default implementations are provided by Wren
3. Applications can provide custom implementations by:
   ...
   - Configuring Wren to use the custom implementation via configuration
...
# Default implementation in Wren:
...
Wren provides several components that can be extended:
...
If upgrading from a version where these utilities were in `wren.utils`, update your imports:

```python
# Before
from wren.utils.async_utils import call_sync_from_async
from wren.utils.llm import get_supported_llm_models
from wren.utils.import_utils import get_impl
from wren.utils.environment import is_running_in_docker

# After
from wren.app_server.utils.async_utils import call_sync_from_async
from wren.app_server.utils.llm import get_supported_llm_models
from wren.app_server.utils.import_utils import get_impl
from wren.app_server.utils.environment import is_running_in_docker
```

All utilities previously in `wren.utils.*` are now available at `wren.app_server.utils.*`.
