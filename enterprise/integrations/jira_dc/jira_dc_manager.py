...
from wren.app_server.integrations.service_types import Comment, Repository
...
from wren.app_server.utils.http_session import httpx_verify_option
...
# Unicode codepoint of the emoji reaction posted to acknowledge an @wren
...
def _comment_addresses_wren(comment: str, bot_ids: set[str] | None) -> bool:
    # Literal @wren always triggers (works even when the bot identity can't
    ...
    # someone@wren.dev (incl. the service account's own address).
...
class JiraDcManager(Manager[JiraDcViewInterface]):
    ...
    def parse_webhook(
        ...
        if event_type == 'comment_created':
            ...
            if not _comment_addresses_wren(comment, bot_mentions):
                return None
    ...
    async def receive_message(self, message: Message):
        ...
        if not job_context:
            ...
            # bot itself (name 'wren' or a resolved bot id) -- the FDE-84
    ...
    async def _resolve_service_account_mentions(self, payload: Dict) -> set[str] | None:
        ...
        Returns the ids a [~...] token can carry; _comment_addresses_wren
        matches a token's inner id against these. Gated so only wiki-markup
        mentions pay a lookup; literal @wren and non-comment payloads
    ...
    async def _add_acknowledgement_reaction(
        ...
        """Acknowledge the @wren mention with a best-effort reaction.
