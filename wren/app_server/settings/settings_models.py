...
class MarketplaceRegistration(BaseModel):
    ...
    Wire-compatible with ``wren.sdk.marketplace.MarketplaceRegistration``
...
def _load_persisted_agent_settings(
    ...
    ``agent_kind: 'llm'`` tag to ``'wren'``, and validates against the
...
class Settings(BaseModel):
    ...
    def _mode_must_be_valid(cls, v: str) -> str:
        if v not in ('code', 'video'):
            raise ValueError(f"mode must be 'code' or 'video', got {v!r}")
        return v
