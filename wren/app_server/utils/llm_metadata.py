...
def should_set_litellm_extra_body(model_name: str) -> bool:
    ...
    Only set litellm_extra_body for wren models to avoid issues
    with providers that don't support extra_body parameters.

    The SDK translates "wren/" models to the LiteLLM proxy only when
    ...
    """
    return model_name.startswith('wren/')
...
def get_llm_metadata(
    ...
    """
    wren_version = wren.__version__

    metadata: dict[str, Any] = {
        'trace_version': wren_version,
        'tags': [
            'app:wren',
            f'model:{model_name}',
            f'type:{llm_type}',
            f'web_host:{os.environ.get("WEB_HOST", "unspecified")}',
            f'wren_version:{wren_version}',
