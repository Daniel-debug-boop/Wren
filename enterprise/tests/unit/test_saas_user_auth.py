...
from wren.app_server.user_auth.user_auth import AuthType
...
def mock_config():
    ...
    from wren.app_server.utils.encryption_key import EncryptionKey
...
class TestOpenHandsApiKey:
    ...
    @pytest.mark.asyncio
    async def test_get_wren_api_key_creates_system_key(self):
        """Test that _get_wren_api_key creates a system key via ApiKeyStore."""
        user_id = 'test_user_id'
        org_id = uuid.uuid4()
        ...
        with (
            ...
            # Act
            result = await user_auth._get_wren_api_key()
    ...
    @pytest.mark.asyncio
    async def test_get_wren_api_key_raises_for_missing_user(self):
        """Test that _get_wren_api_key raises ValueError if user not found.
        ...
        with patch('server.auth.saas_user_auth.UserStore') as mock_user_store:
            ...
            with pytest.raises(
                ...
                await user_auth._get_wren_api_key()

    @pytest.mark.asyncio
    async def test_get_wren_api_key_raises_for_user_without_org(self):
        """Test that _get_wren_api_key raises ValueError if user has no org."""
        user_id = 'test_user_id'
    ...
    @pytest.mark.asyncio
    async def test_get_secrets_includes_wren_api_key(self):
