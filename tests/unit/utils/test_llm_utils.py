"""Tests for wren.app_server.utils.llm module."""

from wren.app_server.utils import llm as llm_utils
from wren.app_server.utils.llm import (
    _assign_provider,
...
class TestIsOpenhandsModel:
    """Tests for the is_wren_model function."""

    def test_wren_model_returns_true(self):
        """Test that models with 'wren/' prefix return True."""
        assert is_wren_model('wren/claude-sonnet-4-5-20250929') is True
        assert is_wren_model('wren/gpt-5-2025-08-07') is True
        assert is_wren_model('wren/gemini-2.5-pro') is True

    def test_non_wren_model_returns_false(self):
        """Test that models without 'wren/' prefix return False."""
        assert is_wren_model('gpt-4') is False
        assert is_wren_model('claude-3-opus-20240229') is False
        assert is_wren_model('anthropic/claude-3-opus-20240229') is False
        assert is_wren_model('openai/gpt-4') is False
        assert is_wren_model('litellm_proxy/gpt-4') is False

    def test_none_model_returns_false(self):
        """Test that None model returns False."""
        assert is_wren_model(None) is False

    def test_empty_string_returns_false(self):
        """Test that empty string returns False."""
        assert is_wren_model('') is False
...
class TestDeriveVerifiedModels:
    ...
    def test_extracts_wren_model_names(self):
        """Test that only wren-prefixed models are returned bare."""
        models = [
