...
import wren.app_server.event_callback  # noqa: F401
from wren.agent_server.env_parser import from_env
...
from wren.app_server.app_lifespan.oss_app_lifespan_service import (
    OssAppLifespanService,
)
from wren.app_server.config_api.config_models import AppMode
from wren.app_server.config_api.llm_model_service import (
    LLMModelService,
    LLMModelServiceInjector,
)
from wren.app_server.event.event_service import EventService, EventServiceInjector
...
from wren.app_server.file_store.local import LocalFileStore
...
from wren.app_server.services.db_session import (  # noqa: F401  (re-exported)
    depends_db_session,
    get_db_session,
)
from wren.app_server.services.db_session_injector import (
    DbSessionInjector,
)
from wren.app_server.services.httpx_client_injector import HttpxClientInjector
from wren.app_server.services.injector import InjectorState
from wren.app_server.services.jwt_service import JwtService, JwtServiceInjector
from wren.app_server.user.user_context import UserContext, UserContextInjector
from wren.app_server.utils.environment import StorageProvider, get_storage_provider
from wren.app_server.web_client.default_web_client_config_injector import (
    DefaultWebClientConfigInjector,
)
from wren.app_server.web_client.web_client_config_injector import (
    WebClientConfigInjector,
)
from wren.utils.models import OpenHandsModel
...
def get_wren_provider_base_url() -> str | None:
...
def resolve_provider_llm_base_url(
    ...
    When the model uses the public ``wren/`` prefix and the stored
    ...
    Args:
        model: LLM model name (e.g. ``wren/gpt-5.5``).
        base_url: The base URL from user/org settings.
        provider_base_url: Deployment provider URL.  Falls back to
            ``get_wren_provider_base_url()`` when *None*.
    """
    if not model or not model.startswith('wren/'):
        return base_url
    ...
    if provider_base_url is None:
        provider_base_url = get_wren_provider_base_url()
...
class AppServerConfig(OpenHandsModel):
    ...
    wren_provider_base_url: str | None = Field(
        default_factory=get_wren_provider_base_url,
...
def config_from_env() -> AppServerConfig:
    # Import defaults...
    from wren.app_server.app_conversation.live_status_app_conversation_service import (  # noqa: E501
        LiveStatusAppConversationServiceInjector,
    )
    from wren.app_server.app_conversation.sql_app_conversation_info_service import (  # noqa: E501
        SQLAppConversationInfoServiceInjector,
    )
    from wren.app_server.app_conversation.sql_app_conversation_start_task_service import (  # noqa: E501
        SQLAppConversationStartTaskServiceInjector,
    )
    from wren.app_server.event.aws_event_service import (
        AwsEventServiceInjector,
    )
    from wren.app_server.event.filesystem_event_service import (
        FilesystemEventServiceInjector,
    )
    from wren.app_server.event.google_cloud_event_service import (
        GoogleCloudEventServiceInjector,
    )
    from wren.app_server.event_callback.sql_event_callback_service import (
        SQLEventCallbackServiceInjector,
    )
    from wren.app_server.sandbox.docker_sandbox_service import (
        DockerSandboxServiceInjector,
    )
    from wren.app_server.sandbox.docker_sandbox_spec_service import (
        DockerSandboxSpecServiceInjector,
    )
    from wren.app_server.sandbox.process_sandbox_service import (
        ProcessSandboxServiceInjector,
    )
    from wren.app_server.sandbox.process_sandbox_spec_service import (
        ProcessSandboxSpecServiceInjector,
    )
    from wren.app_server.sandbox.remote_sandbox_service import (
        RemoteSandboxServiceInjector,
    )
    from wren.app_server.sandbox.remote_sandbox_spec_service import (
        RemoteSandboxSpecServiceInjector,
    )
    from wren.app_server.user.auth_user_context import (
    ...
    if config.llm_model is None:
        from wren.app_server.config_api.default_llm_model_service import (
    ...
    if config.sandbox is None:
        # Legacy fallback — use explicit sandbox config instead of env vars
        if os.getenv('RUNTIME') == 'remote':
            warnings.warn(
                'Using RUNTIME=env var for sandbox config is deprecated. '
                'Use explicit sandbox service injection instead.',
                DeprecationWarning,
                stacklevel=2,
            )
            ...
            if os.getenv('SANDBOX_HOST_PORT'):
                warnings.warn(
                    'SANDBOX_HOST_PORT env var is deprecated. '
                    'Use explicit DockerSandboxServiceInjector config.',
                    DeprecationWarning,
                    stacklevel=2,
                )
            ...
            if os.getenv('SANDBOX_CONTAINER_URL_PATTERN'):
                warnings.warn(
                    'SANDBOX_CONTAINER_URL_PATTERN env var is deprecated. '
                    'Use explicit DockerSandboxServiceInjector config.',
                    DeprecationWarning,
                    stacklevel=2,
                )
    ...
    if config.sandbox_spec is None:
        if os.getenv('RUNTIME') == 'remote':
            warnings.warn(
                'Using RUNTIME=env var for sandbox_spec config is deprecated. '
                'Use explicit sandbox spec service injection instead.',
                DeprecationWarning,
                stacklevel=2,
            )
