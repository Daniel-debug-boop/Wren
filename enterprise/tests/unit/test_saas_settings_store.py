...
from wren.app_server.settings.settings_models import Settings as DataSettings
...
async def test_ensure_api_key_keeps_valid_key():
    ...
    item = _make_settings(model='wren/gpt-4', api_key=existing_key)
    ...
    with patch(
        ...
        await store._ensure_api_key(item, 'org-123', wren_type=True)
...
async def test_ensure_api_key_generates_new_key_when_verification_fails():
    ...
    switching to/from an wren/* model never orphans a key."""
    from storage.lite_llm_manager import get_wren_cloud_key_alias

    store = SaasSettingsStore('test-user-id-123')
    new_key = 'sk-new-key'
    item = _make_settings(model='wren/gpt-4', api_key='sk-invalid-key')
    expected_alias = get_wren_cloud_key_alias('test-user-id-123', 'org-123')
    ...
    with (
        ...
        # The wren branch now deletes the prior key under the shared alias
        ...
        mock_generate.assert_awaited_once_with(
            'test-user-id-123', 'org-123', expected_alias, {'type': 'wren'}
        )
...
async def test_load_canonicalizes_legacy_litellm_proxy_active_llm(
    ...
    assert loaded.agent_settings.llm.model == 'wren/claude-opus-4-8'
...
async def test_load_canonicalizes_legacy_litellm_proxy_llm_profiles(
    ...
    assert legacy.model == 'wren/claude-opus-4-8'
...
@pytest.mark.asyncio
async def test_store_keeps_wren_managed_keys_member_specific(
    session_maker, async_session_maker, org_with_multiple_members_fixture
):
    ...
    with session_maker() as session:
        org = session.execute(select(Org).where(Org.id == org_id)).scalars().first()
        assert org is not None
        # Settings keeps the public wren/ provider prefix in persisted data
        assert (
            org.agent_settings['llm']['model'] == 'wren/claude-opus-4-5-20251101'
        )
...
@pytest.mark.asyncio
async def test_store_skips_ensure_api_key_for_non_wren_model_without_base_url(
    session_maker, async_session_maker, org_with_multiple_members_fixture
):
...
@pytest.mark.asyncio
async def test_store_calls_ensure_api_key_for_wren_model_without_base_url(
    session_maker, async_session_maker, org_with_multiple_members_fixture
):
