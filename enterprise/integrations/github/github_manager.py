...
class GithubManager(Manager[GithubViewType]):
    ...
    def _send_user_not_found_message(self, message: Message, username: str):
        ...
        - Labeled issues (action='labeled' with wren label)
        - Issue comments (comment containing @wren)
        - PR comments (comment containing @wren on a PR)
        - Inline PR review comments (comment containing @wren)
    ...
    async def is_job_requested(self, message: Message) -> bool:
        ...
        # Suggestions contain `@wren` macro; avoid kicking off jobs for system recommendations
    ...
    async def start_job(self, github_view: GithubViewType) -> None:
        """Kick off a job with wren agent using V1 app conversation system.
