...
from wren.app_server.integrations.service_types import Repository
...
OPENHANDS_RESOLVER_TEMPLATES_DIR = (
    ...
    or 'wren/app_server/integrations/templates/resolver/'
)
...
def get_oh_labels(web_host: str) -> tuple[str, str]:
    ...
    Returns:
        ...
        - oh_label is OH_RESOLVER_LABEL when set; otherwise 'wren-exp' for
          staging/local hosts and 'wren' for everything else
    ...
    oh_label = 'wren-exp' if is_staging_or_local else 'wren'
    inline_oh_label = '@wren-exp' if is_staging_or_local else '@wren'
...
def has_exact_mention(text: str, mention: str) -> bool:
    ...
    Args:
        ...
        mention: The mention to look for (e.g. "@wren")

    Returns:
        bool: True if the exact mention is found, False otherwise

    Example:
        >>> has_exact_mention("Hello @wren!", "@wren")  # True
        >>> has_exact_mention("Hello @wren-agent!", "@wren")  # False
        >>> has_exact_mention("(@wren)", "@wren")  # True
        >>> has_exact_mention("user@wren.com", "@wren")  # False
        >>> has_exact_mention("Hello @OpenHands!", "@wren")  # True (case-insensitive)
    """
