...
async def require_financial_data_access(
    ...
    2. User has @wren.dev email domain
    ...
    # Check if user has @wren.dev email
    user_auth = await get_user_auth(request)
    user_email = await user_auth.get_user_email()

    if user_email and user_email.endswith('@wren.dev'):
        logger.debug(
            'Financial data access granted via @wren.dev email',
