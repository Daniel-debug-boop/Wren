...
from wren.agent_server.models import Success
from wren.analytics import get_analytics_service, resolve_analytics_context
...
from wren.app_server.sandbox.sandbox_spec_service import SandboxSpecService
from wren.app_server.services.db_session_injector import set_db_session_keep_open
from wren.app_server.services.httpx_client_injector import (
...
from wren.app_server.settings.settings_models import Settings
from wren.app_server.settings.settings_router import LITE_LLM_API_URL
from wren.app_server.user.specifiy_user_context import USER_CONTEXT_ATTR
from wren.app_server.user.user_context import UserContext
from wren.app_server.user_auth import get_user_settings
...
async def get_conversation_skills(
    ...
    - User skills (~/.wren/skills/)
    - Organization skills (org/.wren repository)
    - Repository skills (repo .agents/skills/, .wren/microagents/, and legacy .wren/skills/)
    ...
    try:
        ...
        for skill in all_skills:
            ...
            if isinstance(skill.trigger, (KeywordTrigger, TaskTrigger)):
                if isinstance(skill.trigger, TaskTrigger):
                    triggers = skill.trigger.triggers or []
                else:
                    triggers = skill.trigger.keywords or []
...
async def get_conversation_hooks(
    ...
    workspace (i.e. `{project_dir}/.wren/hooks.json`) at request time.

    Note:
        This is intentionally a "live" view of the workspace configuration.
        If `.wren/hooks.json` changes over time, this endpoint reflects the
    ...
    try:
        ...
        try:
            ...
            if analytics and user_id:
                from wren.analytics.analytics_context import AnalyticsContext
