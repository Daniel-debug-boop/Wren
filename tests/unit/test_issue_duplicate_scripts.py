...
def test_extract_duplicate_metadata_and_veto_helpers():
    ...
    assert module.extract_duplicate_metadata(
        '<!-- wren-duplicate-check canonical=42 auto-close=true -->'
    ) == (42, True)
...
def test_find_latest_auto_close_comment_prefers_newest_timestamp():
    ...
    comments = [
        {
            'body': '<!-- wren-duplicate-check canonical=10 auto-close=true -->',
            'created_at': '2026-04-20T00:00:00Z',
            'id': 1,
        },
        {
            'body': '<!-- wren-duplicate-check canonical=11 auto-close=true -->',
...
def test_find_latest_auto_close_comment_returns_latest_candidate():
    ...
    comments = [
        {'body': 'plain comment'},
        {
            'body': '<!-- wren-duplicate-check canonical=10 auto-close=false -->',
            'id': 1,
        ...
        {
            'body': '<!-- wren-duplicate-check canonical=12 auto-close=true -->',
            'id': 3,
            'created_at': '2026-04-20T00:00:00Z',
        },
    ]
...
def test_auto_close_main_honors_author_veto(monkeypatch, capsys):
    ...
    comments = [
        {
            'id': 11,
            'body': '<!-- wren-duplicate-check canonical=45 auto-close=true -->',
            'created_at': old_timestamp,
        }
    ]
...
def test_fetch_app_server_events_ignores_non_list_items(monkeypatch):
    ...
    monkeypatch.setattr(
        module, 'wren_headers', lambda: {'Authorization': 'Bearer test-token'}
    )
