...
# skills/ is at the repo root, two levels above the wren package __file__
GLOBAL_SKILLS_DIR = Path(wren.__file__).parent.parent / 'skills'
USER_SKILLS_DIR = Path.home() / '.wren' / 'microagents'
...
async def _clone_marketplace_repo(
    ...
    try:
        clone_dir = Path(
            tempfile.mkdtemp(prefix=f'wren_marketplace_{marketplace.name}_')
        )
