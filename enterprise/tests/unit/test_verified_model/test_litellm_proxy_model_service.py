...
from wren.app_server.utils.llm import ModelsResponse
...
HAPPY_PAYLOAD = {
    'data': [
        ...
        },
        {
            ...
            'model_info': {
                'wren_hidden': True,
                'wren_canonical': 'claude-sonnet-4-5',
            },
        },
        {
...
# guard; an ``wren/``-prefixed entry must be dropped from the union.
...
class TestHappyPath:
    async def test_models_response_shape(self, monkeypatch):
        ...
        # wren/ prefix.
        assert response.models == ['wren/claude-sonnet-4-5', 'wren/gpt-5']
        assert response.verified_models == ['claude-sonnet-4-5', 'gpt-5']
        assert response.verified_providers == ['wren']
        # Hidden alias routes are still reported so saved settings that
        # reference them count as available.
        assert response.hidden_models == ['wren/hidden-model']
        # The alias carries its canonical mapping (canonical is visible).
        assert response.hidden_model_canonicals == {
            'wren/hidden-model': 'wren/claude-sonnet-4-5'
        }
        # litellm_proxy/ prefix translated to wren/.
        assert response.default_model == 'wren/claude-sonnet-4-5'
    ...
    async def test_search_llm_models_uses_proxy_models(self, monkeypatch):
        ...
        assert names == [
            ('wren', 'claude-sonnet-4-5', False, None),
            ('wren', 'gpt-5', False, None),
            ...
            ('wren', 'hidden-model', True, 'claude-sonnet-4-5'),
        ]
    ...
    async def test_hidden_only_when_all_duplicate_entries_hidden(self, monkeypatch):
        ...
        assert response.models == ['wren/mixed-model']
        assert response.hidden_models == ['wren/legacy-alias']
        # No wren_canonical tags anywhere -> no mappings.
        assert response.hidden_model_canonicals == {}
    ...
    async def test_canonical_skipped_when_not_visible(self, monkeypatch):
        ...
        assert response.models == ['wren/real-model']
        assert response.hidden_models == [
            'wren/alias-to-missing',
            'wren/alias-to-hidden',
            'wren/other-alias',
        ]
        assert response.hidden_model_canonicals == {}
    ...
    async def test_canonical_mapped_for_visible_target(self, monkeypatch):
        ...
        assert response.hidden_model_canonicals == {
            'wren/legacy-alias': 'wren/real-model'
        }

    async def test_search_providers_only_wren(self, monkeypatch):
        install_client(monkeypatch, response=FakeResponse(HAPPY_PAYLOAD))
        service = LiteLLMProxyModelService()

        page = await service.search_providers()

        assert [(p.name, p.verified) for p in page.items] == [('wren', True)]
...
class TestDefaultModelTranslation:
    def test_litellm_proxy_prefix_translated(self, monkeypatch):
        ...
        assert _derive_default_model() == 'wren/gpt-5'

    def test_wren_prefix_preserved(self, monkeypatch):
        monkeypatch.setattr(
            router_module, 'get_default_litellm_model', lambda: 'wren/gpt-5'
        )
        assert _derive_default_model() == 'wren/gpt-5'
...
class TestFailurePaths:
    ...
    async def test_stale_cache_served_on_failure(self, monkeypatch, fake_clock, caplog):
        ...
        assert first.models == ['wren/claude-sonnet-4-5', 'wren/gpt-5']
...
class TestCacheTtl:
    ...
    async def test_after_ttl_refetches(self, monkeypatch, fake_clock):
        ...
        assert len(calls) == 1
        assert len(calls2) == 1
        assert response.models == ['wren/new-model']
...
class TestByokUnion:
    ...
    async def test_byok_on_unions_catalogue(self, monkeypatch):
        ...
        # the wren/-prefixed catalogue entry is dropped.
        assert response.models == [
            'wren/claude-sonnet-4-5',
            'wren/gpt-5',
            'anthropic/claude-sonnet-4-5',
            'openai/gpt-5',
            'openai/gpt-4o',
    ...
    async def test_byok_on_only_managed_models_verified(self, monkeypatch):
        ...
        assert verified == {('wren', 'claude-sonnet-4-5'), ('wren', 'gpt-5')}
    ...
    async def test_byok_on_exposes_catalogue_providers(self, monkeypatch):
        ...
        # wren stays the verified managed provider, listed first; catalogue
