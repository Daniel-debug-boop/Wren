...
_run_setup:
	...
	@poetry run uvicorn wren.server.listen:app --host $(BACKEND_HOST) --port $(BACKEND_PORT) &
...
# Phony targets
.PHONY: build check-dependencies check-system check-python check-npm check-nodejs check-docker check-poetry install-python-dependencies install-frontend-dependencies install-pre-commit-hooks lint-backend lint-frontend lint test-frontend test build-frontend start-backend start-frontend _run_setup run run-wsl setup-config setup-config-prompts setup-config-basic wren-cloud-run docker-dev docker-run clean help
.PHONY: kind
