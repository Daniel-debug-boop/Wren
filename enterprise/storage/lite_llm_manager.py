...
def get_wren_cloud_key_alias(keycloak_user_id: str, org_id: str) -> str:
...
class LiteLlmManager:
        ...
        if not local_deploy:
            ...
            async with httpx.AsyncClient(
                ...
                if add_user_to_team:
                    ...
                    key_alias = get_wren_cloud_key_alias(keycloak_user_id, org_id)
                    try:
                ...
                if db_key:
                    ...
                    if not key_valid:
                        ...
                        new_key = await LiteLlmManager._generate_key(
                            ...
                            keycloak_user_id,
                            org_id,
                            get_wren_cloud_key_alias(keycloak_user_id, org_id),
                            None,
                        )
                        logger.info(
    ...
    async def _verify_existing_key(
        ...
        the given user and organization. For wren_type=True, looks for keys
        with metadata type='wren' and matching team_id. For wren_type=False,
