...
- A simple local development setup, with live reloading for both Wren and this repo
...
This option will allow you to modify both the Wren code and the code in this repo,
...
The open source Wren repo should be cloned as a sibling directory,
in `../Wren`. This is hard-coded in the pyproject.toml (edit if necessary)

If you're doing this the first time, you may need to run

```
poetry update wren-ai
```
...
### 3. Start the Wren frontend

Start the frontend like you normally would in the open source Wren repo.
...
```bash
docker run  -p 6379:6379 --name wren-redis -d redis
...
Develop on [Openhands](https://github.com/Wren/Wren) locally. When ready, run the following inside Openhands repo (not the Deploy repo)

```
docker build -f containers/app/Dockerfile -t wren .
```

### 3. Build SAAS Openhands

Build the SAAS image locally inside Deploy repo. Note that `wren` is the name of the image built in Step 2

```
docker build -t wren-saas ./app/ --build-arg BASE="wren"
```
...
This is the last step! Run SAAS wren locally using

```
docker run --env-file ./app/.env -p 3000:3000 wren-saas
```
...
Local Development necessitates running a version of Wren that is as similar as possible to the version running in the SAAS Environment. Before running these steps, it is assumed you have a local development version of Wren running.
...
`docker run -p 5432:5432 --name my-postgres -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=wren -d postgres`
    ...
    "configurations": [
        ...
        },
        {
            ...
            "env": {
                ...
                "FILE_STORE_PATH": "<YOUR HOME DIRECTORY>>/.wren-state",
