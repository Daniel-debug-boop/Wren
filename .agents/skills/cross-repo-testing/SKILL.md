...
description: This skill should be used when the user asks to "test a cross-repo feature", "deploy a feature branch to staging", "test SDK against OH Cloud", "e2e test a cloud workspace feature", "test provider tokens", "test secrets inheritance", or when changes span the SDK and Wren server repos and need end-to-end validation against a staging deployment.
triggers:
- cross-repo
- staging deployment
...
# Cross-Repo Testing: SDK ↔ Wren Cloud

How to end-to-end test features that span `Wren/software-agent-sdk` and `Wren/Wren` (the Cloud backend).

## Repository Map

| Repo | Role | What lives here |
|------|------|-----------------|
| [`software-agent-sdk`](https://github.com/Wren/software-agent-sdk) | Agent core | `wren-sdk`, `wren-workspace`, `wren-tools` packages. `WrenCloudWorkspace` lives here. |
| [`Wren`](https://github.com/Wren/Wren) | Cloud backend | FastAPI server (`wren/app_server/`), sandbox management, auth, enterprise integrations. Deployed as OH Cloud. |
| [`deploy`](https://github.com/Wren/deploy) | Infrastructure | Helm charts + GitHub Actions that build the enterprise Docker image and deploy to staging/production. |
...
In the `Wren` repo, implement the new API endpoint(s). Run unit tests:
...
Push a PR. Wait for the **"Push Enterprise Image" (Docker) CI job** to succeed — this builds `ghcr.io/wren/enterprise-server:sha-<COMMIT>`.

### A2. Write the SDK-side changes

In `software-agent-sdk`, implement the client code (e.g., new methods on `WrenCloudWorkspace`). Run SDK unit tests:

```bash
cd software-agent-sdk
pip install -e wren-sdk -e wren-workspace
pytest tests/ -v
```
...
### B2. Pin SDK packages to the commit in the Wren PR

In the `Wren` repo PR, pin all 3 SDK packages (`wren-sdk`, `wren-agent-server`, `wren-tools`) to the unreleased commit and update the agent-server image tag. This involves editing 3 files and regenerating 3 lock files.
...
### B3. Wait for the Wren enterprise image to build

Push the pinned changes. The Wren CI will build a new enterprise Docker image (`ghcr.io/wren/enterprise-server:sha-<OH_COMMIT>`) that bundles the unreleased SDK. Wait for the "Push Enterprise Image" job to succeed.

### B4. Deploy and test

Follow [Deploying to a Staging Feature Environment](#deploying-to-a-staging-feature-environment) using the new Wren commit SHA.

### B5. Before merging: remove the pin

**CI guard:** `check-package-versions.yml` blocks merge to `main` if `[tool.poetry.dependencies]` contains `rev` fields. Before the Wren PR can merge, the SDK PR must be merged and released to PyPI, then the pin must be replaced with the released version number.

---

## Deploying to a Staging Feature Environment

The `deploy` repo creates preview environments from Wren PRs.

**Option A — GitHub Actions UI (preferred):**
Go to `Wren/deploy` → Actions → "Create Wren preview PR" → enter the Wren PR number. This creates a branch `ohpr-<PR>-<random>` and opens a deploy PR.
...
```bash
gh api repos/Wren/Wren/actions/runs \
...
| **SDK pin blocks merge** | `check-package-versions.yml` prevents merging an Wren PR that has `rev` fields in `[tool.poetry.dependencies]`. The SDK must be released to PyPI first. |
| **Flow A: stock agent-server is fine** | When only the Cloud API changes, `WrenCloudWorkspace` talks to the Cloud server, not the agent-server. No custom image needed. |
