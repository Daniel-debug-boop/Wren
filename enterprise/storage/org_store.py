...
class OrgStore:
    ...
    def get_agent_settings_from_org(org: Org) -> AgentSettingsConfig:
        ...
        # migrations (incl. the legacy ``agent_kind: 'llm'`` -> ``'wren'``
    ...
    async def _maybe_get_managed_llm_key_for_user(
        ...
        wren_type = is_wren_model(llm_model)
        uses_managed_llm_key = (
            normalized_llm_base_url == normalized_managed_base_url
            or (normalized_llm_base_url is None and wren_type)
        )
        ...
        # deleting any prior key first — symmetric across wren/* and BYOR
        # defaults so switching between them never orphans a key.
        key_alias = get_wren_cloud_key_alias(user_id, str(updated_org.id))
