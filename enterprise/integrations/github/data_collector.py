...
def _github_ts_to_naive_utc(value: str | None) -> datetime | None:
    ...
    bind to the ``TIMESTAMP WITHOUT TIME ZONE`` columns on ``wren_prs``
...
class GitHubDataCollector:
    ...
    def _check_wren_author(self, name, login) -> bool:
        return (
            name == 'wren'
            or login == 'wren'
            or login == 'wren-agent'
            or login == 'wren-ai'
            or login == 'wren-staging'
            or login == 'wren-exp'
            or (login and 'wren' in login.lower())
        )
    ...
    def _save_issue(
        ...
        """
        Save issue data when it's labeled with wren
    ...
    def _count_wren_activity(
        ...
        for commit in commits:
            ...
            if self._check_wren_author(author_name, author_login):
                wren_commit_count += 1
        ...
        for pr_comment in pr_comments:
            ...
            author_name = ''  # Initialize to avoid reference before assignment
            if self._check_wren_author(author_name, author_login):
                wren_general_comment_count += 1

        return (
            wren_commit_count,
            wren_review_comment_count,
    ...
    def _build_final_data_structure(
        ...
        wren_commit_count: int,
        wren_review_comment_count: int,
        wren_general_comment_count: int = 0,
    ) -> dict:
        ...
        return {
            ...
            'wren_stats': {
                'num_commits': wren_commit_count,
                'num_review_comments': wren_review_comment_count,
                'num_general_comments': wren_general_comment_count,
                'helped_author': wren_commit_count > 0,
            },
        }
    ...
    async def save_full_pr(self, wren_pr: WrenPR) -> None:
        ...
        - Merge status
        - Num wren commits
        - Num wren review comments
        """
        pr_number = wren_pr.pr_number
        if wren_pr.installation_id is None:
            logger.warning(
                f'Skipping PR {wren_pr.repo_name}#{pr_number}: missing installation_id'
            )
            return
        installation_id = int(wren_pr.installation_id)
        repo_id = wren_pr.repo_id
        ...
        try:
            ...
            logger.warning(
                f'Failed to generate token for {wren_pr.repo_name}: {e}'
            )
            return
        ...
        (
            ...
            wren_review_comment_count,
            wren_general_comment_count,
        ) = self._count_wren_activity(commits, review_comments, pr_comments)

        logger.info(
            f'[Github]: PR #{pr_number} - OpenHands commits: {wren_commit_count}, review comments: {wren_review_comment_count}, general comments: {wren_general_comment_count}'
        )
        logger.info(
        ...
        update_success = await store.update_pr_wren_stats(
            repo_id=repo_id,
            pr_number=pr_number,
            original_updated_at=wren_pr.updated_at,
            wren_helped_author=wren_helped_author,
            num_wren_commits=wren_commit_count,
            num_wren_review_comments=wren_review_comment_count,
            num_wren_general_comments=wren_general_comment_count,
        )
        ...
        self._save_data(file_name, data)
        logger.info(
            f'[Github]: Saved full PR #{pr_number} for repo {repo_id} with OpenHands stats: commits={wren_commit_count}, reviews={wren_review_comment_count}, general_comments={wren_general_comment_count}, helped={wren_helped_author}'
        )
    ...
    async def _track_closed_or_merged_pr(self, payload):
        ...
        # TIMESTAMP columns on wren_prs (see _github_ts_to_naive_utc).
        ...
        pr = WrenPR(
            ...
            wren_helped_author=None,
            num_wren_commits=None,
            num_wren_review_comments=None,
