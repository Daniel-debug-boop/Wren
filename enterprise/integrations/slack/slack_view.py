...
from wren.app_server.config import get_app_conversation_service
from wren.app_server.integrations.provider import ProviderHandler
from wren.app_server.sandbox.sandbox_models import SandboxStatus
...
from wren.app_server.utils.async_utils import GENERAL_TIMEOUT
from wren.app_server.utils.logger import wren_logger as logger
from wren.sdk import ImageContent, TextContent
...
class SlackNewConversationView(SlackViewInterface):
    ...
    async def _get_instructions(self, jinja_env: Environment) -> tuple[str, str]:
        ...
        user_info: SlackUser = self.slack_to_wren_user
    ...
    async def create_or_update_conversation(self, jinja: Environment) -> str:
        ...
        if self._resolved_git_provider and self.selected_repo:
            self.resolved_org_id = await resolve_org_for_repo(
                ...
                keycloak_user_id=self.slack_to_wren_user.keycloak_user_id,
            )
    ...
    async def _create_v1_conversation(self, jinja: Environment) -> None:
        ...
        start_request = AppConversationStartRequest(
            ...
            title=f'Slack conversation from {self.slack_to_wren_user.slack_display_name}',
...
class SlackUpdateExistingConversationView(SlackNewConversationView):
    ...
    async def send_message_to_v1_conversation(self, jinja: Environment):
        ...
        from wren.agent_server.models import SendMessageRequest
        from wren.app_server.config import (
            get_app_conversation_info_service,
            get_httpx_client,
            get_sandbox_service,
        )
        from wren.app_server.event_callback.util import (
...
class SlackFactory:
    ...
    async def create_slack_view_from_payload(
        ...
        # Determine if this is a known slack user by wren
