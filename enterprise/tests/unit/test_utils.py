...
def test_has_exact_mention():
    ...
    assert has_exact_mention('Hello @wren!', '@wren') is True
    assert has_exact_mention('@wren at start', '@wren') is True
    assert has_exact_mention('End with @wren', '@wren') is True
    assert has_exact_mention('@wren', '@wren') is True

    # Test no match
    assert has_exact_mention('No mention here', '@wren') is False
    assert has_exact_mention('', '@wren') is False

    # Test partial matches (should be False)
    assert has_exact_mention('Hello @wren-agent!', '@wren') is False
    assert has_exact_mention('Email: user@wren.com', '@wren') is False
    assert has_exact_mention('Text@wren', '@wren') is False
    assert has_exact_mention('@wrenmore', '@wren') is False
    ...
    assert has_exact_mention('Hi @OpenHands', '@wren') is True
    assert has_exact_mention('Hi @wren', '@OpenHands') is True
    assert has_exact_mention('Hi @OPENHANDS', '@wren') is True

    # Test multiple mentions
    assert has_exact_mention('@wren and @wren again', '@wren') is True
    assert has_exact_mention('@wren-agent and @wren', '@wren') is True

    # Test with surrounding punctuation
    assert has_exact_mention('Hey, @wren!', '@wren') is True
    assert has_exact_mention('(@wren)', '@wren') is True
    assert has_exact_mention('@wren: hello', '@wren') is True
    assert has_exact_mention('@wren? yes', '@wren') is True
...
def test_infer_repo_from_message():
    test_cases = [
        ...
        (
            '@wren-exp just echo hello world in `OpenHands/OpenHands-CLI` repository',
            ['OpenHands/OpenHands-CLI'],
        ),
        (
            '@wren-exp echo hello world with {{OpenHands/OpenHands-CLI}}',
            ['OpenHands/OpenHands-CLI'],
        ),
