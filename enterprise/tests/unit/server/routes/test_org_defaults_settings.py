...
def test_normalize_agent_settings_masks_api_key_in_json_on_empty_and_real_keys():
    ...
    empty_key = OrgUpdate.model_validate(
        {
            'agent_settings_diff': {
                'llm': {'model': 'wren/x', 'api_key': '', 'base_url': None},
            },
        }
    )
...
def test_normalize_agent_settings_fills_base_url_for_all_providers():
    ...
    wren_null = OrgUpdate.model_validate(
        {
            'agent_settings_diff': {
                'llm': {'model': 'wren/claude-3', 'base_url': None},
            },
        }
    )
    wren_missing = OrgUpdate.model_validate(
        {'agent_settings_diff': {'llm': {'model': 'wren/claude-3'}}}
    )
    ...
    wren_null_diff = wren_null.agent_settings_diff
    assert wren_null_diff is not None
    assert wren_null_diff['llm']['model'] == 'wren/claude-3'
    assert wren_null_diff['llm']['base_url'].rstrip('/') == (
        LITE_LLM_API_URL.rstrip('/')
    )

    wren_missing_diff = wren_missing.agent_settings_diff
    assert wren_missing_diff is not None
    assert wren_missing_diff['llm']['model'] == 'wren/claude-3'
    assert wren_missing_diff['llm']['base_url'].rstrip('/') == (
        LITE_LLM_API_URL.rstrip('/')
    )
...
def test_from_org_validates_persisted_wren_agent_kind():
    """GIVEN: An org row whose persisted ``agent_settings`` carry the
        canonical ``agent_kind: 'wren'`` discriminator (the exact shape
    ...
    org.agent_settings = {
        ...
        'agent': 'CodeActAgent',
        'agent_kind': 'wren',
        'llm': {'model': 'wren/claude', 'base_url': LITE_LLM_API_URL},
    }
    ...
    # Assert
    assert response.agent_settings.agent_kind == 'wren'
    assert response.agent_settings.llm.model == 'wren/claude'
...
def test_from_org_keeps_wren_prefix_and_hides_managed_base_url():
    ...
    assert response.agent_settings.llm.model == 'wren/minimax-m2.5'
...
def test_from_org_returns_provider_default_base_url_as_stored_for_non_managed_models():
    ...
    from wren.app_server.utils.llm import get_provider_api_base as _provider_base
